...
function add_css() {
	...
	style.id = "svelte-1lj6xle-style";
	style.textContent = "section.svelte-1lj6xle{margin:0 1em;width:calc(50vw - 7em)}iframe.svelte-1lj6xle{width:100%;border:1px solid var(--color-neutral-secondary-default);border-radius:.125rem;display:block;height:50vh;margin-bottom:2em}";
...
function create_fragment(ctx) {
	...
	if (/*contentPath*/ ctx[0] !== void 0) {
		afinput_props.value = /*contentPath*/ ctx[0];
	}
	...
	return {
		c() {
			...
			if (iframe_1.src !== (iframe_1_src_value = /*path*/ ctx[1])) attr(iframe_1, "src", iframe_1_src_value);
			attr(iframe_1, "title", "Component viewer");
			attr(iframe_1, "class", "svelte-1lj6xle");
			attr(section, "class", "svelte-1lj6xle");
		},
		m(target, anchor) {
		...
		p(ctx, [dirty]) {
			if (!current || dirty & /*path*/ 2 && iframe_1.src !== (iframe_1_src_value = /*path*/ ctx[1])) {
				attr(iframe_1, "src", iframe_1_src_value);
			}

			const afinput_changes = {};

			if (!updating_value && dirty & /*contentPath*/ 1) {
				updating_value = true;
				afinput_changes.value = /*contentPath*/ ctx[0];
				add_flush_callback(() => updating_value = false);
			}

			afinput.$set(afinput_changes);
			const aftextarea_changes = {};
			if (dirty & /*contentPath*/ 1) aftextarea_changes.disabled = !!/*contentPath*/ ctx[0];
...
function instance($$self, $$props, $$invalidate) {
	...
	? `${contentPath}?viewpath=${getCorePath(_componentPath)}&time=${new Date().toISOString()}`
	: _componentPath;
	...
	function displayData(value, timestamp) {
		try {
			...
			$$invalidate(1, path = `${_componentPath}?time=${timestamp}&content=${data}`);
	...
	function refresh() {
		...
		if (contentPath) {
			$$invalidate(1, path = `${contentPath}?viewpath=${getCorePath(_componentPath)}&time=${timestamp}`);
		} else if (jsonData) {
			displayData(jsonData, timestamp);
		} else {
			$$invalidate(1, path = `${_componentPath}?time=${timestamp}`);
		}
	}
	...
	$$self.$$set = $$props => {
		...
		if ("contentPath" in $$props) $$invalidate(0, contentPath = $$props.contentPath);
	};
...
class Component extends SvelteComponent {
	constructor(options) {
		super();
		if (!document.getElementById("svelte-1lj6xle-style")) add_css();
