<script>
  ...
  EditorStore.subscribe((v) => (tree = v.tree));
  ...
  const addInCurrentFolder = () => {
    ...
    const addPath =
      currentSelected.type === 'Directory'
  ...
  <ul class="toolbar">
    <li>
      <AFButton
        ...
        icon="plus"
        size="small"
        on:click={addInCurrentFolder}>Add</AFButton
      >
    </li>
    <li>
      <AFButton
        ...
        icon="trash"
        size="small"
        on:click={deleteSelected}>Remove</AFButton
      >
    </li>
    <li>
      <AFButton
        ...
        icon="refresh"
        size="small"
        on:click={refresh}>Refresh</AFButton
      >
    </li>
  </ul>
  <nav>
    <ul>
      {#each tree as item}
        <li class="level-1"><NavItem {item} /></li>
      {/each}
    </ul>
  </nav>
</div>
