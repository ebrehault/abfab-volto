<script>
  ...
  $: {
    if (componentPath) {
      link =
        location.origin +
        (contentPath ? getRealPath(contentPath) : getRealPath(componentPath));
...
<section>
      ...
      <AFDropdown label="Publish" icon="pencil">
        <div class="publish-buttons">
          <AFButton
            kind="primary"
            aspect="basic"
            on:click={() => share('Allow')}>Publish</AFButton
          >
          <AFButton
            kind="destructive"
            aspect="basic"
            on:click={() => share('Deny')}>Unpublish</AFButton
          >
          <AFButton
            kind="secondary"
            aspect="basic"
            on:click={() => share('Unset')}>Reset</AFButton
          >
        </div>
      </AFDropdown>
    </p>
  ...
  {#if componentPath}
    ...
    <p>
      <a target="_new" href={link}
        >Open in its own tab <AFIcon size="small" icon="external" /></a
      >
    </p>
    <h3>Embed</h3>
  ...
  {#if hasGit}
    ...
    <div class="sync-action">
      <div>
        <h4>Push</h4>
        <p>
          All the changes made in this directory will be pushed to the <code
            >{branch}</code
          > branch on your Git repository.
        </p>
        <AFInput
          bind:value={commit}
    ...
    <div class="action-container">
      <div>
        <h4>Pull</h4>
        <p>
          The current code from <code>{branch}</code> branch on your Git
          repository will be deployed here. If some changes have been made
          through this online inerface since the last syncing, they will be
          pushed to the <code>abfab-online-backup</code> branch before any code is
          deployed, so nothing gets lost by mistake.
        </p>
      </div>
      <div>
        <AFButton
  ...
  {#if isLib}
    ...
    <p>
      Allows to install a library and all its dependencies in the <code
        >/node_modules</code
      > directory.
    </p>
    <div class="action-container install-section">
      ...
      <AFButton
        ...
        icon="arrow-down"
        on:click={install}
        disabled={!lib || installing}>Install</AFButton
      >
    </div>
