<script>
  ...
  import { AbFabStore, redirectToLogin } from '/++api++/~/abfab/core.js';
  import { onMount, onDestroy } from 'svelte';
  import { derived } from '/++api++/~/node_modules/svelte/store';
  ...
  subscriptions.push(
    derived(AbFabStore, (state) => state.logged).subscribe((isLogged) => {
      if (!isLogged) {
        redirectToLogin();
      }
    }),
...
<svelte:head>
  <link rel="stylesheet" href="/++api++/~/abfab/pastanaga/pastanaga.css" />
</svelte:head>
<header>
...
<main>
  ...
  {/if}
  <div
    class="editor-container {play || properties ? 'half' : ''}"
    ...
    {#if hasError}
      <div class="errors-container">
        ...
        <div class="errors">
          {#if error}
            <div class="error">
              {error.message}<code>{error.frame}</code>
            </div>{/if}
  ...
  {#if play}
    <Viewer bind:this={viewer} componentPath={viewComponent} {contentPath} />
  {/if}
  {#if properties}
    <Properties componentPath={viewComponent} {contentPath} {hasGit} />
  {/if}
</main>
